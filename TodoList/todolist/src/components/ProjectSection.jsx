...
export const ProjectSection = ({ expanded }) => {
  ...
  const onRename = () => {
    if (projectName.trim()) {
      ...
      setIsEditing(false)
      setProjectName("")
      setProjectEditingId(null) // Reset ID
    }
  }
        ...
        <div>
          {projects.map((project) => {
            return (
              <div
                key={project.id}
                className="relative flex gap-4 pl-8 items-center text-white font-black text-lg bg-gradient-blue  px-2 py-2"
              >
                ...
                {showOptionsId === project.id && (
                  <div className="absolute bg-white text-black rounded shadow-lg mt-2 right-8 top-[-150%]">
